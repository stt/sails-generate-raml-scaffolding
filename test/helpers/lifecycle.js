	...
	setup: function (scope) {
		return function _before (done) {
			...
			_cleanup(reportback.extend({
				success: function () {
          // fake a sails app
          fs.mkdirSync(scope.rootPath);
          fs.writeFileSync(scope.rootPath+"/package.json", "{}");
          fs.mkdirSync(scope.rootPath+"/config");
          fs.writeFileSync(scope.rootPath+"/config/blueprints.js",
            "module.exports.blueprints = {pluralize:true};");
          sailsgen(scope, done);
				}
			}));
		};
